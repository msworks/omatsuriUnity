...
public class clOHHB_V23 : clZ80RAM
{
	...
	//========================================================================================
	//入力パラメータ HL ﾚｼﾞｽﾀ  :  ｱﾄﾞﾚｽ ﾃﾞｰﾀ ﾃｰﾌﾞﾙ 番地
	//出力パラメータ HL ﾚｼﾞｽﾀ  :  新 ﾃﾞｰﾀ ﾃｰﾌﾞﾙ 番地
	//使用レジスタ   DE ﾚｼﾞｽﾀ  :  汎用
	//              HL        :  汎用
	//========================================================================================
	...
	//========================================================================================
	//入力パラメータ A  ﾚｼﾞｽﾀ  :  図柄位置 ｶｳﾝﾀｰ
	//              HL        :  図柄配置 ﾃｰﾌﾞﾙ ､ 停止 ﾃｰﾌﾞﾙ
	//出力パラメータ A  ﾚｼﾞｽﾀ  :  図柄位置の ﾃﾞｰﾀ
	//              E         :  図柄位置 ｶｳﾝﾀｰ
	//              HL        :  図柄配置 ﾃｰﾌﾞﾙ ､ 停止 ﾃｰﾌﾞﾙ
	//破壊レジスタ A ､ E ､ HL ﾚｼﾞｽﾀ
	//========================================================================================
...
//
    ...
    /// リールストップ関数（ストップボタン）
    /// </summary>
    /// <param name="reel">回転中に停止されたリール（0:左/1:中/2:右）</param>
    /// <param name="pos">停止したリールのｾﾝﾀｰﾗｲﾝ位置番号（0～20）</param>
    /// <returns>最終リール位置番号（0～20）</returns>
    public static ushort mReelStop(int reel, ushort pos)
	{
        ushort[] REEL_PIC = {
            (Defines.DEF_RCB_PIC + 0),
            (Defines.DEF_RCB_PIC + 1),
            (Defines.DEF_RCB_PIC + 2)
        };
        ushort[] REEL_STP = {
            (Defines.DEF_RCB_POS + 0),
            (Defines.DEF_RCB_POS + 1),
            (Defines.DEF_RCB_POS + 2)
        };
        ushort bit = 0x01;
		int i = reel;
    ...
    /// 入賞＆メダル払出し関数
    /// </summary>
    /// <returns>払い出しメダル数（0～15）0xFFFFの場合はエラー</returns>
